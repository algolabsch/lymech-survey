# -*- coding: utf-8 -*-
"""
Created on Thu Jul 24 10:45:19 2025

@author: master
"""

# ======== WARNING -> THIS IS A WEB STREAMLIT SURVEY, SHOULD BE RUN IN CMD LINE WITH FOLLOWING 2 SEQUENCES -> ============
...
# https://medium.com/@achanandhi.m/docker-compose-build-what-it-is-and-when-you-need-it-a79788f42fce
...
pages = survey.pages(9, progress_bar=True, 
                     on_submit=lambda: st.success("Vos réponses sont enregistrées, merci de votre collaboration !"))

# ------ permanent title ----------------------------------
st.title('Lyme Suisse - Questionnaire')
backgroundColor = "#B0CEF5"
...
with pages:
    ...
    if pages.current == 0:
        ...
        st.session_state['uuid'] = [str(uuid.uuid4()) for _ in range(2)]
    
        #st.session_state.res = [str(uuid.uuid4()) for _ in range(2)]
        st.write("UUID :"+str(st.session_state['uuid']))
        ...
        # KEY INITIALIZATION - Session State also supports attribute based syntax
        # if 'Q100' not in st.session_state:
        #     st.session_state.Q100 = ''
        
        # if 'Q101' not in st.session_state:
        #     st.session_state.Q101 = ''
        ...
        survey.radio('''Autorisez-vous Lyme Suisse à  utiliser vos réponses pour des raisons analytiques (Recherche)?*''', 
                     ...
                     horizontal=True,
                     key = "Q101")
        st.write("Q100 : ", st.session_state.Q100)
        st.write("Q101 : ", st.session_state.Q101)
        
        Q_100 = st.session_state.Q100
        Q_101 = st.session_state.Q101
        
        # df_quest['q1'] = st.session_state.Q100
        # df_quest['q2'] = st.session_state.Q101
    # ------------------------------------------------------------------------------------------
        ...
        st.date_input("Current time : ", ct, key = "Q102")
        #st.write("Your birthday is:", d)
           
        survey.number_input('Quel est votre âge ?*', key = "Q103")
        
        survey.radio('Votre sexe ?*', options=["Masculin", "Féminin","Autre"], index=None, key = 'Q104', horizontal=True)
        
        #st.write('Votre âge : ', number)
        st.write("You selected yr sex : ", st.session_state.Q104)
        # Selection Boxes
        # Selection box:
        ...
        # Choose options
        # survey.multiselect("Multiple choice:", options=["Option 1", "Option 2", "Option 3",
        
        survey.selectbox('Quel est votre lieu (canton) de résidence ?*', options=["Vaud", "Fribourg", "Neuchâtel",'Genève','Bern','Tessin','Bern', 'Hors Suisse'], key = 'q105')
        
        # survey.selectbox('Quel est votre lieu (canton) de résidence ?*',("Vaud", "Fribourg", "Neuchâtel",'Genève','Bern','Tessin','Bern', 'Hors Suisse'))
        st.write("You selected the residence : ", st.session_state.q105)
        ...
        nb_duration_disease = survey.number_input('Durée de la maladie (en années): *', key = 'q106')
        nb_duration_wandering = survey.number_input("Durée de l'errance  médicale (en années): *")
        num_years_to_diag = survey.slider("Nb d'année jusqu'au premier diagnostique (en années) ", min_value=1, max_value=50)
        
        # survey.multiselect("Multiple choice:", options=["Option 1", "Option 2", "Option 3", "etc"])
        
        survey.multiselect(
        "Avez-vous, EN SUISSE, reçu des diagnostics différents de la maladie de Lyme ou co-infections (complétez ci-dessous) ?",
        options=['Fibromyalgie','Syndrome de fatigue chronique', 'Troubles psychosomatiques','Autres maladies infectieuses',
         ...
         "Schizophrénie",'Psychoses','Autre','Non'], key = 'q107' )
        
        survey.multiselect(
        "Avez-vous, à l'étranger (HORS SUISSE), reçu des diagnostics différents de la maladie de Lyme ou co-infections (complétez ci-dessous) ?",
        options=['Fibromyalgie','Syndrome de fatigue chronique', 'Troubles psychosomatiques','Autres maladies infectieuses',
         'Maladie auto-immune','Dépression','Troubles anxieux',"Thada", "Autisme", "Dépression",
         "Schizophrénie",'Psychoses','Autre','Non'] , key = 'q108' )
        ...
        st.session_state.num_rows_ch = survey.slider('CH - Number of rows', min_value=1, max_value=10)
        ...
        def add_row(row):
            ...
            with grid[0]:
                ...
                survey.selectbox("Pathogène recherché",
                                 options=['Borrelia','Bartonella','Babesia','encephalitis virus (TBE-V)','Anaplasma','Rickettsia','Ehrlichia','Autre virus','Autre parasite','Autre Bacteria'],
                                 index=None,
...
# Loop to create rows of input widgets
    ...
    # ================================================================================  
        ...
        private_tests = survey.radio('''Avez-vous effectué des tests privés (Ã  l'étranger par ex.) et reçu un diagnostic clair de maladie de Lyme
                                      (et/ou de coinfections Ã©ventuelles) par un laboratoire ou un mÃ©decin ?''',
                                      options=['Oui', 'Non'], key = 'q110', index=None,  horizontal=True)
        ...
        # a selection for the user to specify the number of rows
        st.session_state.num_rows_nch = survey.slider('NON CH - Number of rows', min_value=1, max_value=10)
        ...
        def add_row(row):
            ...
            with grid[3]:
                ...
                survey.selectbox('Test effectués en (Région, pays)',
                                 options=['Suisse', 'EU (hors Suisse)','USA','Autre'],
                                 index=None,
...
# Loop to create rows of input widgets
    ...
    # ================================================================================                                                                                                        
        ...
        survey.multiselect("Possédez-vous (ou possédiez) des animaux de compagnie ?", options=['Aucun','Chien','Chat','Autres (mammifères)','Autres (non mammifères)'], key="Q63" )
        ...
        options=["l'entretien de zones naturelles ?",'autre',"l'assistance aux animaux domestiques (SPA, chenils, refuges, ..) ?",
...
# ----------- test -----------------------------------------------
#import json
#json_string = json.dumps(" ברי צקלה  , test avec é ou  è ou à et où.. confirmer.. ")
#json_string.encode('ascii').decode('unicode-escape')
#Out[2]: '" ברי צקלה  , test avec é ou  è ou à et où.. confirmer.. "'




# -------------WRITING DATA TO JSON FILE ---------------------
# --- convert survey to json and show data ------------
...
# create data -> valid Json object 
data = survey.to_json()

#json_data = json.dumps(data, ensure_ascii=False)
# --- write data directly on webpage...
st.json(data)

# -----save json string data to file
path_json = "D:/_00_LYME_CH/data/lyme_ch_questionnaire/"
ctstr = str(st.session_state.Q102)
ipstr = str(st.session_state.ip)
filedir = path_json + 'lyme_ch_data_quest_' + ipstr + '_' + ctstr + '.json'

    
with open(filedir, 'w') as json_file:
    # json.dump(data, json_file2, encoding="utf8")
